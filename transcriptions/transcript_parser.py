    ...
    def get_verse_details(self, ab_element, context_info):
        ...
        if context_info in ['incipit', 'explicit']:
            ...
            try:
                ...
                raise
            info_dict['book_number'] = int(info_dict['book_number'])
            ...
            context = 'B%02dK%02dV%02d' % (info_dict['book_number'], info_dict['chapter_number'], info_dict['verse_number'])
...
class LectionaryParser(TEIParser):

    #we overwrite all of the parent version here
    #siglum is not affected by the positional info unless there are repeated verses in the same lection
    def check_duplicate_verses(self, verses):
        multiple_counts = {}
        processed_verse_ids = []
        processed_sigla = []
        multiple_sigla_counts = {}
        for verse in verses:
            #check within the whole lectionary
            if verse['identifier'] in processed_verse_ids:
                if verse['identifier'] in multiple_counts.keys():
                    multiple_counts[verse['identifier']] += 1
                else:
                ...
                verse['identifier'] = '%s_%s_%s-%s_%s' % (
                        ...
                        self.lang.upper(),
                        self.siglum,
                        multiple_counts[verse['identifier']],  verse['context'])

                #now check within each lection
                if '%s-%s' % (verse['context'], verse['siglum']) in processed_sigla: #this will never happen unless the verse is also repeated so okay to be within this if statement
                    if '%s-%s' % (verse['context'], verse['siglum']) in multiple_sigla_counts.keys():
                        multiple_sigla_counts['%s-%s' % (verse['context'], verse['siglum'])] += 1
                    else:
                        multiple_sigla_counts['%s-%s' % (verse['context'], verse['siglum'])] = 2
                    verse['siglum'] = '%s-%s' % (verse['siglum'], multiple_sigla_counts['%s-%s' % (verse['context'], verse['siglum'])])

            processed_sigla.append('%s-%s' % (verse['context'], verse['siglum']))
            processed_verse_ids.append(verse['identifier'])
        #now check whether we need to add -1 to any of the ids
        if len(multiple_counts.keys()) > 0:
            for verse in verses:
                if verse['identifier'] in multiple_counts.keys():
                    verse['identifier'] = '%s-1%s' % (
                        verse['identifier'][:verse['identifier'].rfind('_')],
                        verse['identifier'][verse['identifier'].rfind('_'):])
                    verse['duplicate_position'] = 1

        #and to any of the sigla
        if len(multiple_sigla_counts.keys()) > 0:
            for verse in verses:
                if '%s-%s' % (verse['context'], verse['siglum']) in multiple_sigla_counts.keys():
                    verse['siglum'] = '%s-1' % verse['siglum']
        return verses
    ...
    #add lectionary specific data to the verse details
    #calling parent function to get basic stuff
    #we overwrite siglum here because we want to append lection details
    def get_verse_details(self, ab_element, context_info):

        verse_info = super(LectionaryParser, self).get_verse_details(ab_element, context_info)
        try:
            lection_div = ab_element.xpath('.//ancestor::tei:div[@type="lection"]', namespaces=self.nsmap)[0]
        except:
            verse_info['siglum'] = self.siglum
            print('this verse is not in a lection - %s' % verse_info['n'])
        else:
            verse_info['siglum'] = '%s-%s' % (self.siglum, lection_div.get('n'))
            verse_info['lection'] = lection_div.get('n')
