...
class Command(BaseCommand):
    ...
    def add_transcription_to_database(self, filename, language, collection):
        ...
        xml_parser = YasnaParser(file_string, filename=filename, corpus=collection, private=False)

        data, private = xml_parser.get_data() #returns a tuple
        siglum = data['transcription']['siglum']

        print(siglum)
        print(data['transcription']['work_id'])


        # #book_number = int(work_id.replace('NT_B', ''))
        # works = Work.objects.all().filter(collection__abbreviation=collection, abbreviation=work_id)
        # if len(works) == 1:
        #     work_id = works[0].id
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument( '--collection', action='store', dest='collection', help='The collection the transcription/s should be assigned to.')
    ...
    def handle(self, *args, **options):
        ...
        if os.path.isfile(location):
            self.add_transcription_to_database(location, language, collection)
        else:
            for root, directory, files in os.walk(location):
                for filename in files:
                    if filename.endswith('.xml'):
                        self.add_transcription_to_database(os.path.join(root, filename), language, collection)
